...
export const adminService = {
  ...
  getUsers: async (params: {
    ...
    const res = await authorizedAxiosInstance.get<{
      data: { users: UserListDTO[]; pagination: any }
    }>('/admin/users', { params })
    return res.data.data
  },
  ...
  getPermissions: async () => {
    const res = await authorizedAxiosInstance.get<{ data: PermissionDTO[] }>(
      '/admin/roles/permissions'
    )
    return res.data.data
  },
  getRolePermissions: async (id: string) => {
    const res = await authorizedAxiosInstance.get<{ data: string[] }>(
      `/admin/roles/${id}/permissions`
    )
    return res.data.data
  },
  updateRolePermissions: async (id: string, permissionIds: string[]) => {
    const res = await authorizedAxiosInstance.put(`/admin/roles/${id}/permissions`, {
      permissionIds,
    })
    return res.data
  },
  ...
  getBroadcasts: async (params: { page?: number; limit?: number }) => {
    const res = await authorizedAxiosInstance.get<{ data: { broadcasts: any[]; pagination: any } }>(
      '/admin/notifications',
      { params }
    )
    return res.data.data
  ...
  getLogs: async (params: { page?: number; limit?: number; action?: string }) => {
    const res = await authorizedAxiosInstance.get<{ data: { logs: AdminLog[]; pagination: any } }>(
      '/admin/logs',
      { params }
    )
    return res.data.data
  ...
  getTrash: async (params: { type: string; page?: number; limit?: number }) => {
    const res = await authorizedAxiosInstance.get<{ data: { items: any[]; pagination: any } }>(
      '/admin/trash',
      { params }
    )
    return res.data.data
  ...
  getSystemVocabs: async (params: any) => {
    const res = await authorizedAxiosInstance.get<{ data: any; meta: any }>('/admin/vocab', {
      params,
    })
    return res.data
  },
  ...
  getUserResults: async (userId: string, params: { page?: number; limit?: number }) => {
    const res = await authorizedAxiosInstance.get<{ data: { results: any[]; pagination: any } }>(
