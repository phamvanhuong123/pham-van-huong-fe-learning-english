...
export const vocabService = {
  async getVocabs(query: VocabQuery) {
    const res = await authorizedAxiosInstance.get<{ data: Vocab[]; meta: any }>(API_ROOT, {
      params: query,
    })
    return res.data
  },
  ...
  async getVocabTopics(): Promise<string[]> {
    const res = await authorizedAxiosInstance.get<{ data: string[] }>(`${API_ROOT}/topics`)
