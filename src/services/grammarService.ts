...
export const grammarService = {
  ...
  getAdminTopicById: async (id: string) => {
    const response = await authorizedAxiosInstance.get<{ data: GrammarTopic }>(
      `/admin/grammar/${id}`
    )
  ...
  createTopic: async (data: CreateGrammarTopicInput) => {
    const response = await authorizedAxiosInstance.post<{ data: GrammarTopic }>(
      '/admin/grammar',
      data
    )
    return response.data.data
  ...
  updateTopic: async (id: string, data: UpdateGrammarTopicInput) => {
    const response = await authorizedAxiosInstance.put<{ data: GrammarTopic }>(
      `/admin/grammar/${id}`,
      data
    )
    return response.data.data
  ...
  getTopicQuestions: async (topicId: string) => {
    const response = await authorizedAxiosInstance.get<{
      data: { topic: GrammarTopic; questions: AdminGrammarQuestion[] }
  ...
  startPracticeSession: async (slug: string) => {
    const response = await authorizedAxiosInstance.post<{ data: GrammarPracticeStartResponse }>(
      `/grammar/${slug}/start`
    )
    return response.data.data
  },
  ...
  submitAnswer: async (
    sessionId: string,
    data: { questionId: string; selectedLabel: string; timeTakenSeconds: number }
  ) => {
    const response = await authorizedAxiosInstance.post<{ data: GrammarAnswerResponse }>(
      `/grammar/session/${sessionId}/answer`,
      data
    )
    return response.data.data
  },
