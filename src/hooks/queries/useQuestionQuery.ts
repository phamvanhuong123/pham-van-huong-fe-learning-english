...
export const useSaveQuestion = (onSuccess?: () => void) => {
  ...
  return useMutation({
    mutationFn: async ({ data, isGroup, editingId }: SavePayload) => {
      if (editingId) {
        return isGroup ? updatePassageGroupApi(editingId, data) : updateQuestionApi(editingId, data)
      }
      return isGroup ? createQuestionGroupApi(data) : createStandaloneQuestionApi(data)
    },
