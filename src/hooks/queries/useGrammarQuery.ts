...
export const useGetAdminGrammarTopics = (params?: {
  page?: number
  limit?: number
  search?: string
}) => {
...
export const useCreateGrammarTopic = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [...grammarKeys.all, 'admin'] })
    },
  })
}
...
export const useUpdateGrammarTopic = () => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateGrammarTopicInput }) =>
      grammarService.updateTopic(id, data),
...
export const useCreateGrammarQuestion = (topicId: string) => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (data: CreateGrammarQuestionInput) =>
      grammarService.createTopicQuestion(topicId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: grammarKeys.topicQuestions(topicId) })
      queryClient.invalidateQueries({ queryKey: [...grammarKeys.all, 'admin'] }) // cập nhật _count
    },
  })
}
