...
export const subscriptionKeys = {
  ...
  adminList: (filters: SubscriptionQueryFilters) =>
    [...subscriptionKeys.all, 'admin', filters] as const,
}
...
export const useRejectSubscription = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: RejectSubscriptionDto }) =>
      subscriptionService.rejectSubscription(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: subscriptionKeys.adminList({}) })
    },
  })
...
export const useEditSubscription = () => {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: { plan: string; amount: number } }) =>
      adminService.editPendingSubscription(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: subscriptionKeys.adminList({}) })
    },
  })
...
export const useRevokeSubscription = () => {
  ...
  return useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      adminService.revokeSubscription(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: subscriptionKeys.adminList({}) })
    },
  })
