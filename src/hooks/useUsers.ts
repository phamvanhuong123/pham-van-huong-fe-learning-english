...
export const useUsers = (filters: { page?: number; limit?: number; search?: string; role?: string; status?: string }) => {
  return useQuery({
...
export const useBanUser = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(variables.isBanned ? 'Đã khóa tài khoản thành công' : 'Đã mở khóa tài khoản thành công');
    },
    onError: (error: any) => {
