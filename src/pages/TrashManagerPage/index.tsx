        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow>
                <TableCell colSpan={4} className="h-40 text-center text-rose-500">Lỗi khi tải dữ liệu thùng rác.</TableCell>
              </TableRow>
            ) : data?.items.length === 0 ? (
              <TableRow>
                <TableCell colSpan={4} className="h-40">
                  <AdminEmptyState title="Thùng rác trống" description="Không có dữ liệu nào bị xóa gần đây." icon="trash" />
                </TableCell>
              </TableRow>
            ) : (
              data?.items.map((item) => (
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={() => handleOpenRestore(item.id, item.title || item.questionText || '')} className="cursor-pointer">
                          <RotateCcw className="mr-2 h-4 w-4 text-emerald-600" /> Khôi phục
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleOpenDelete(item.id, item.title || item.questionText || '')} className="text-destructive focus:text-destructive cursor-pointer">
