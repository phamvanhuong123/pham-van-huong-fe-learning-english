    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8">
      ...
      <div className="flex flex-col gap-6 lg:border-r lg:pr-8 h-fit lg:sticky lg:top-24 max-h-[calc(100vh-8rem)] overflow-y-auto custom-scrollbar">
        {passageGroup.passages.map((passage, idx) => (
          <div key={passage.id} className="passage-content">
            {passage.mediaType === 'IMAGE' && passage.mediaUrl && (
              <img src={passage.mediaUrl} alt={`Passage ${idx + 1}`} className="w-full h-auto rounded-lg border mb-4" />
            )}
            {passage.content && (
              <div
                className="prose prose-blue max-w-none text-gray-800 leading-relaxed bg-gray-50 p-6 rounded-lg border"
