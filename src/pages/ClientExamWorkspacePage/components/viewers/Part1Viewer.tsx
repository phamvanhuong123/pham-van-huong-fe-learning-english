...
export const Part1Viewer: React.FC<Part1ViewerProps> = ({ passageGroup, question }) => {
  ...
  const audioPassage = passageGroup?.passages.find(
    (p) => p.mediaType === 'AUDIO' || p.mediaType === 'audio'
  )
  const imagePassage = passageGroup?.passages.find(
    (p) => p.mediaType === 'IMAGE' || p.mediaType === 'image'
  )
  const videoPassage = passageGroup?.passages.find(
    (p) => p.mediaType === 'VIDEO' || p.mediaType === 'video'
  )
  ...
  return (
    <div
      ...
      className="bg-white p-3 md:p-4 rounded-xl shadow-sm border border-slate-100 mb-3 scroll-mt-20"
    >
