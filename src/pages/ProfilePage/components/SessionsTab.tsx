...
import { getSessionsApi, revokeAllOtherSessionsApi, revokeSessionApi } from '@/services/sessionService';
    ...
    <div className="p-6 md:p-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Các thiết bị hiện đang đăng nhập vào tài khoản của bạn</p>
        </div>
        {otherSessions.length > 0 && (
          <Button variant="outline" onClick={handleRevokeAll} disabled={revokingAll} className="whitespace-nowrap transition-colors hover:bg-destructive/10 hover:text-destructive hover:border-destructive">
            {revokingAll ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ShieldAlert className="mr-2 h-4 w-4" />}
      ...
      <div className="space-y-4">
        {sessions.map((session) => {
          ...
          return (
            <div key={session.id} className="flex items-start sm:items-center justify-between p-4 rounded-xl border bg-card transition-colors hover:bg-muted/30">
              <div className="flex items-start sm:items-center gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h4 className="font-medium text-sm sm:text-base">{parseDevice(session.deviceInfo)}</h4>
                    {session.isCurrent && (
                      <Badge variant="secondary" className="bg-green-500/10 text-green-600 hover:bg-green-500/20 text-xs font-medium border-0">Thiết bị này</Badge>
                    )}
                  </div>
