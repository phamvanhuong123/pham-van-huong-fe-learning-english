...
export const ExamCard: React.FC<ExamCardProps> = ({ exam }) => {
  ...
  const getDifficultyColor = (diff: string) => {
    switch(diff) {
      case 'EASY': return 'bg-emerald-50 text-emerald-700 border-emerald-200/60';
      case 'MEDIUM': return 'bg-amber-50 text-amber-700 border-amber-200/60';
      case 'HARD': return 'bg-rose-50 text-rose-700 border-rose-200/60';
    ...
    <Card className={cn(
      ...
      <CardHeader className="pb-4 pt-6 px-6">
        <div className="flex justify-between items-center mb-4">
          <Badge variant="secondary" className="bg-slate-100/80 text-slate-700 font-semibold tracking-wide text-xs px-2.5 py-1 border-none">
            {exam.part === 'FULL' ? 'FULL TEST' : exam.part.replace('PART', 'PART ')}
          </Badge>
          
          <div className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border", getDifficultyColor(exam.difficulty))}>
