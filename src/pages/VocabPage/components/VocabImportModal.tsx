...
import { useImportVocabCsv } from '@/hooks/queries/useVocabQuery'
...
export default function VocabImportModal({ isOpen, onClose }: VocabImportModalProps) {
  const [file, setFile] = useState<File | null>(null)
  const [importResult, setImportResult] = useState<any>(null)
  const fileInputRef = useRef<HTMLInputElement>(null)

  const { mutateAsync: importCsv, isPending } = useImportVocabCsv()
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (res.data.errorCount > 0) {
        toast.warning(
          `Import thành công ${res.data.successCount} từ, lỗi ${res.data.errorCount} từ`
        )
      } else {
        toast.success(`Import thành công toàn bộ ${res.data.successCount} từ`)
      }
    } catch (error) {
  ...
  const handleExportMyVocab = () => {
    window.location.href = vocabService.exportCsvUrl()
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Import / Export Từ vựng</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          <div className="flex gap-2">
            ...
            <Button variant="outline" className="flex-1 flex gap-2" onClick={handleExportMyVocab}>
              <Download className="h-4 w-4" /> Export từ của tôi
            </Button>
          </div>
          ...
          {importResult && (
            <Alert
              variant={importResult.errorCount > 0 ? 'destructive' : 'default'}
              className={
                importResult.errorCount === 0 ? 'bg-green-50 text-green-800 border-green-200' : ''
              }
            >
                    ...
                    <>
                      ...
                      <div className="max-h-24 overflow-y-auto text-xs mt-2 p-2 bg-background/50 rounded border">
                        {importResult.errors.map((e: any, i: number) => (
                          <div key={i}>
                            Dòng {e.row}: {JSON.stringify(e.error)}
                          </div>
                        ))}
                      </div>
                    </>
