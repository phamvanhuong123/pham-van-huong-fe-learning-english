...
const statsConfig = [
  ...
  { key: 'REVIEW', label: 'Chờ ôn tập', color: 'bg-orange-100 text-orange-800' },
  { key: 'MASTERED', label: 'Đã thuộc', color: 'bg-green-100 text-green-800' },
] as const;


interface VocabStatsBarProps {
  activeStatus?: VocabStatus | 'ALL';
  onStatusClick?: (status: VocabStatus | 'ALL') => void;
}

export default function VocabStatsBar({ activeStatus = 'ALL', onStatusClick }: VocabStatsBarProps) {
      ...
      {statsConfig.map(({ key, label, color }) => {
        ...
        return (
          <button
            ...
            className={`flex-1 p-4 rounded-lg flex flex-col items-center justify-center transition-all ${color} ${isActive ? 'ring-2 ring-offset-2 ring-current scale-105 shadow-md' : 'hover:scale-105 opacity-80 hover:opacity-100'}`}
          >
