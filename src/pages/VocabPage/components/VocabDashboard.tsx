                ...
                <div className="flex justify-between items-center mb-4">
                  <div className="flex gap-3">
                    <div className="flex flex-col items-center">
                      ...
                      <span className={`text-sm font-bold px-2 py-1 rounded ${stat.newCount > 0 ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'text-muted-foreground opacity-50'}`}>
                        {stat.newCount}
                      </span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="text-xs text-muted-foreground font-medium mb-1">Đang học</span>
                      <span className={`text-sm font-bold px-2 py-1 rounded ${stat.learningCount > 0 ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'text-muted-foreground opacity-50'}`}>
                        {stat.learningCount}
                      </span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="text-xs text-muted-foreground font-medium mb-1">Ôn tập</span>
                      <span className={`text-sm font-bold px-2 py-1 rounded ${stat.reviewCount > 0 ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'text-muted-foreground opacity-50'}`}>
                        {stat.reviewCount}
                      </span>
                    </div>
                  </div>
