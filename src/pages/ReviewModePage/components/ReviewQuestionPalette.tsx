...
export const ReviewQuestionPalette: React.FC<ReviewQuestionPaletteProps> = ({
  questions,
  activePart,
  onNavigateToPart,
}) => {
    ...
    <div className="bg-white rounded-lg shadow-sm border p-4 flex flex-col h-[calc(100vh-140px)]">
      ...
      <div className="flex items-center justify-between mb-4 text-xs font-medium text-slate-600 px-1">
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 bg-green-500 inline-block rounded-full"></span> Đúng
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 bg-red-500 inline-block rounded-full"></span> Sai
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-3 h-3 bg-white border-2 border-slate-300 inline-block rounded-full"></span>{' '}
          Bỏ qua
        </div>
      </div>
