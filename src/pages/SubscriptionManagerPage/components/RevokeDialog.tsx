...
export default function RevokeDialog({ isOpen, onClose, subscription }: { isOpen: boolean; onClose: () => void; subscription: any }) {
  ...
  const handleRevoke = async () => {
    ...
    try {
      ...
      if (banAccount && subscription.bankAccountNo) {
        await banAccountMutate({ bankAccountNo: subscription.bankAccountNo, reason: 'Gian lận: ' + reason });
      }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Hành động này sẽ trừ đi số ngày VIP tương ứng từ tài khoản học viên và đẩy học viên ra khỏi hệ thống.
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">Lý do thu hồi <span className="text-destructive">*</span></label>
            <textarea 
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer border p-3 rounded-md bg-destructive/5 hover:bg-destructive/10 transition-colors">
            ...
            <div className="flex flex-col">
              <span className="font-semibold text-destructive">Đưa luôn STK này vào danh sách đen</span>
              <span className="text-xs text-muted-foreground">STK: {subscription.bankAccountNo}</span>
            </div>
          </label>
        </div>
        ...
        <DialogFooter>
          ...
          <Button variant="destructive" onClick={handleRevoke} disabled={isPending}>Xác nhận Thu hồi</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
