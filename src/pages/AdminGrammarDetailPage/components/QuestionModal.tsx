...
export function QuestionModal({ isOpen, onClose, topicId, question }: QuestionModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: CreateGrammarQuestionInput = {
      ...
      options: filledOptions.map(o => ({ label: o.label, text: o.text.trim(), isCorrect: o.isCorrect })),
    };
  ...
  const difficultyConfig = {
    EASY: { label: 'Dễ', class: 'text-emerald-600' },
    MEDIUM: { label: 'Trung bình', class: 'text-amber-600' },
    HARD: { label: 'Khó', class: 'text-rose-600' },
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5 py-2">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {correctLabel && (
                <Badge variant="secondary" className="text-emerald-600 bg-emerald-50 border-emerald-200 text-xs gap-1">
              ...
              {options.map(opt => (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    opt.isCorrect ? 'border-emerald-500 bg-emerald-500' : 'border-muted-foreground/40 group-hover:border-primary/60'
                  )}>
                  ...
                  <Input
                    value={opt.text}
                    onChange={e => { e.stopPropagation(); handleOptionTextChange(opt.label, e.target.value); }}
            ...
            <p className="text-xs text-muted-foreground">Click vào một hàng để chọn làm đáp án đúng</p>
          </div>
