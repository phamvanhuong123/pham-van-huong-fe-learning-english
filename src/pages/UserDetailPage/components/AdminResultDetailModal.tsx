      ...
      <DialogContent className="max-w-5xl h-[85vh] flex flex-col p-0 gap-0">
        <DialogHeader className="p-6 border-b pb-4 bg-muted/20">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 mt-2 text-sm text-muted-foreground">
            <div>
              <span className="font-medium text-foreground">Học viên:</span>{' '}
              {result?.user?.name || result?.user?.email}
            </div>
            <div>
              <span className="font-medium text-foreground">Đề thi:</span> {result?.exam?.title}
            </div>
            <div>
              <span className="font-medium text-foreground">Điểm:</span> {result?.score} (Đúng:{' '}
                    ...
                    {result.resultDetails
                      ...
                      .map((detail: any) => {
                        ...
                        return (
                          <div
                            key={detail.id}
                            className={`p-4 rounded-xl border ${detail.isCorrect ? 'border-emerald-200 bg-emerald-50/30' : 'border-rose-200 bg-rose-50/30'}`}
                          >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                  {(q.options || []).map((opt: any) => {
                                    ...
                                    return (
                                      <div
                                        key={opt.id}
                                        className={`p-2 rounded-lg flex items-center gap-2 ${bgClass}`}
                                      >
