    ...
    <div className="p-8 space-y-6 animate-fade-in max-w-7xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-3xl font-bold tracking-tight flex items-center gap-3">
          Chi tiết Bài làm
          {result?.isFullTest && <Badge variant="default" className="bg-blue-600 text-sm">Full Test</Badge>}
        </h2>
      </div>
      ...
      <div className="bg-card border rounded-2xl p-6 shadow-sm">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div>
            ...
            <p className="font-semibold text-emerald-600">{result?.correctQ} / {result?.totalQ}</p>
          </div>
          {result?.isFullTest && (
            <>
              <div>
      ...
      <div className="bg-card border rounded-2xl shadow-sm overflow-hidden">
        <Tabs defaultValue={parts[0]} className="w-full">
          ...
          <div className="p-6 bg-muted/5 min-h-[500px]">
            {parts.map(part => (
              <TabsContent key={part} value={part} className="space-y-6 mt-0 focus-visible:outline-none">
                {result.resultDetails.filter((d: any) => {
                  ...
                  return (
                    <div key={detail.id} className={`p-6 rounded-xl border-2 transition-all ${detail.isCorrect ? 'border-emerald-200/50 bg-emerald-50/30 hover:border-emerald-300' : 'border-rose-200/50 bg-rose-50/30 hover:border-rose-300'}`}>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 space-y-4">
                          {q.passageGroup?.passages?.length > 0 && (
                            <div className="space-y-4 mb-4">
                              {q.passageGroup.passages.map((passage: any) => (
                                <div key={passage.id} className="p-4 bg-background rounded-xl border border-border shadow-sm">
                                  {passage.mediaType === 'AUDIO' && passage.mediaUrl && (
                                    <audio controls src={passage.mediaUrl} className="w-full mb-3 h-10 outline-none" />
                                  )}
                                  {passage.mediaType === 'IMAGE' && passage.mediaUrl && (
                                    <img src={passage.mediaUrl} alt="Question Media" className="max-w-full rounded-lg mb-3 max-h-80 object-contain border" />
                                  )}
                                  {passage.content && (
                                    <div className="prose prose-sm dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: passage.content }} />
                                  )}
                                  {passage.transcript && (
                                    <div className="mt-3 p-3 bg-muted/30 rounded-lg text-sm border border-border/50">
                                      <span className="font-semibold text-primary block mb-1">Transcript / Bản dịch:</span>
                                      <div className="whitespace-pre-wrap leading-relaxed" dangerouslySetInnerHTML={{ __html: passage.transcript }} />
                                    </div>
                                  )}
                                </div>
                              ))}
                          ...
                          <div className="font-medium text-lg leading-relaxed">
                            ...
                            {q.questionText || <span className="text-muted-foreground italic">Câu hỏi hình ảnh / audio</span>}
                          </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-base">
                            {(q.options || []).map((opt: any) => {
                              ...
                              if (isSelected && isCorrect) bgClass = "bg-emerald-100 border-emerald-400 text-emerald-900 font-medium shadow-sm";
                              else if (isSelected && !isCorrect) bgClass = "bg-rose-100 border-rose-400 text-rose-900 font-medium shadow-sm";
                              else if (!isSelected && isCorrect) bgClass = "bg-emerald-50/50 border-emerald-300 border-dashed text-emerald-700";

                              return (
                                <div key={opt.id} className={`p-3 rounded-xl border flex items-start gap-3 transition-colors ${bgClass}`}>
                                  <span className={`font-bold w-7 h-7 flex-shrink-0 flex items-center justify-center rounded-full text-sm ${isSelected ? 'bg-white/80' : 'bg-muted'}`}>
                                    {opt.label}
                                  </span>
                          ...
                          {!detail.selectedLabel && (
                            <div className="text-sm font-medium text-rose-500 bg-rose-50 p-3 rounded-lg border border-rose-100 mt-4 flex items-center gap-2">
                              ...
                              Học viên bỏ trống câu này. Đáp án đúng là <span className="font-bold">{correctOption?.label}</span>.
                            </div>
                          )}
                        </div>
                      </div>
