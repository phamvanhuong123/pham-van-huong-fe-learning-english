...
import {
  ...
  useSubmitGrammarAnswer,
  useEndPracticeSession,
} from '@/hooks/queries/useGrammarQuery'
...
function GrammarPracticePage() {
  ...
  const progressPercent =
    ((currentIndex + (answerResult ? 1 : 0)) / sessionData.questions.length) * 100
      ...
      <div className="max-w-2xl mx-auto py-12 px-4 text-center">
        ...
        >
          ...
          <p className="text-xl text-muted-foreground mb-8">
            ...
            <span className="font-semibold text-foreground">{sessionData.topic.name}</span>
          </p>
              ...
              {currentQuestion?.options.map((option) => {
                ...
                if (answerResult) {
                  if (option.label === answerResult.correctLabel) {
                    optionStateClass =
                      'border-green-500 bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 cursor-default'
                    Icon = CheckCircle2
        ...
        {answerResult && (
          ...
          >
            <div className="flex items-start gap-4">
              <div
                className={`p-2 rounded-full mt-1 ${answerResult.isCorrect ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}
              >
                {answerResult.isCorrect ? (
              ...
              <div>
                <h3
                  className={`text-xl font-bold mb-2 ${answerResult.isCorrect ? 'text-green-700 dark:text-green-400' : 'text-destructive'}`}
                >
                ...
                {answerResult.explanation ? (
                  ...
                  <p className="text-muted-foreground flex items-center gap-2">
                    <AlertCircle className="h-4 w-4" /> Không có giải thích chi tiết cho câu hỏi
                    này.
                  </p>
                )}
              </div>
